...
if ($error_msg && !$is_admin)
    alert(_t("회원아이디")." '{$error_msg}' "._t("은(는) 존재(또는 정보공개)하지 않는 회원아이디 이거나 탈퇴, 접근차단된 회원아이디 입니다.")."\\n"._t("쪽지를 발송하지 않았습니다."));
    ...
    if (count($member_list['id'])) {
        ...
        if ($point) {
            if ($member['mb_point'] - $point < 0) {
                alert(_t('보유하신 포인트(').number_format($member['mb_point'])._t('점)가 모자라서 쪽지를 보낼 수 없습니다.'));
            }
        }
    }
}
...
if ($member_list) {
    ...
    alert($str_nick_list." "._t("님께 쪽지를 전달하였습니다."), G5_HTTP_BBS_URL."/memo.php?kind=send", false);
} else {
    alert(_t("회원아이디 오류 같습니다."), G5_HTTP_BBS_URL."/memo_form.php", false);
}
?>
