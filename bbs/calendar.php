...
// 요일
$yoil = array (_t("일"), _t("월"), _t("화"), _t("수"), _t("목"), _t("금"), _t("토"));
...
for ($i=$fr_yyyy; $i<=$to_yyyy; $i++)
{
    ...
    $yyyy_select .= "<option value='{$i}'{$selected}>$i "._t("년")."</option>";
}
$yyyy_select .= "</select>";
...
for ($i=1; $i<=12; $i++)
{
    ...
    $mm_select .= "<option value='{$i}'{$selected}>$i "._t("월")."</option>";
}
$mm_select .= "</select>";

///$member_skin_path = "$g5[path]/skin/member/$config[cf_member_skin]";
$member_skin_path = "$g5[path]/skin/member/g4_basic";
...
function date_send(year, month, day, wday, handay)
{
    ...
    opener.document.getElementById('<?php echo $fld?>').value = year + delimiter + month + delimiter + day;
    window.close();
}
</script>
