...
if($board['bo_download_point'] < 0 && $is_guest)
    alert(_t('다운로드 권한이 없습니다.').'\\n'._t('회원이시라면 로그인 후 이용해 보십시오.'), G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id));

$sql = " select bf_source, bf_file from {$g5['board_file_table']} where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_no = '$no' ";
$file = sql_fetch($sql);
if (!$file['bf_file'])
    alert_close(_t('파일 정보가 존재하지 않습니다.'));

// JavaScript 불가일 때
if($js != 'on' && $board['bo_download_point'] < 0) {
    $msg = $file['bf_source'].' '._t('파일을 다운로드 하시면 포인트가 차감(').number_format($board['bo_download_point'])._t('점)됩니다.').'\\n'._t('포인트는 게시물당 한번만 차감되며 다음에 다시 다운로드 하셔도 중복하여 차감하지 않습니다.').'\\n'._t('그래도 다운로드 하시겠습니까?');
...
if (!get_session($ss_name))
{
    ...
    else if ($board['bo_download_level'] >= 1) // 회원이상 다운로드가 가능하다면
    {
        ...
        if ($member['mb_point'] + $board['bo_download_point'] < 0)
            alert(_t('보유하신 포인트(').number_format($member['mb_point'])._t(')가 없거나 모자라서 다운로드(').number_format($board['bo_download_point'])._t(')가 불가합니다.').'\\n\\n'._t('포인트를 적립하신 후 다시 다운로드 해 주십시오.'));
...
$g5['title'] = _t('다운로드').' &gt; '.conv_subject($write['wr_subject'], 255);
