...
if ($mb_md5)
{
    if ($mb_md5 == $row['mb_email_certify2'])
    {
        ...
        alert(_t("메일인증 처리를 완료 하였습니다.")."\\n\\n"._t("지금부터")." {$mb_id} "._t("아이디로 로그인 가능합니다."), G5_URL);
    }
    else
    {
        alert(_t('메일인증 요청 정보가 올바르지 않습니다.'), G5_URL);
    }
}

alert(_t('제대로 된 값이 넘어오지 않았습니다.'), G5_URL);
?>
