...
if (!$bo_table) {
    $msg = _t("bo_table 값이 넘어오지 않았습니다.")."\\n\\nboard.php?bo_table=code "._t("와 같은 방식으로 넘겨 주세요.");
    alert($msg);
}
...
if (isset($wr_id) && $wr_id) {
    ...
    if (isset($group['gr_use_access']) && $group['gr_use_access']) {
        if ($is_guest) {
            $msg = _t("비회원은 이 게시판에 접근할 권한이 없습니다.")."\\n\\n"._t("회원이시라면 로그인 후 이용해 보십시오.");
        ...
        if ($is_admin == "super" || $is_admin == "group") {
            ...
            $row = sql_fetch($sql);
            if (!$row['cnt']) {
                alert(_t("접근 권한이 없으므로 글읽기가 불가합니다.")."\\n\\n"._t("궁금하신 사항은 관리자에게 문의 바랍니다."), G5_URL);
            }
        }
    }
    ...
    if ($member['mb_level'] < $board['bo_read_level']) {
        if ($is_member)
            alert(_t('글을 읽을 권한이 없습니다.'), G5_URL);
        else
            alert(_t('글을 읽을 권한이 없습니다.').'\\n\\n'._t('회원이시라면 로그인 후 이용해 보십시오.'), './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr));
    }
    ...
    if ($config['cf_cert_use'] && !$is_admin) {
        ...
        if ($board['bo_use_cert'] != '' && $is_guest) {
            alert(_t('이 게시판은 본인확인 하신 회원님만 글읽기가 가능합니다.').'\\n\\n'._t('회원이시라면 로그인 후 이용해 보십시오.'), './login.php?wr_id='.$wr_id.$qstr.'&amp;url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr));
        }
        ...
        if ($board['bo_use_cert'] == 'adult' && !$member['mb_adult']) {
            alert(_t('이 게시판은 본인확인으로 성인인증 된 회원님만 글읽기가 가능합니다.').'\\n\\n'._t('현재 성인인데 글읽기가 안된다면 회원정보 수정에서 본인확인을 다시 해주시기 바랍니다.'), G5_URL);
        }
    ...
    if (!get_session($ss_name))
    {
        ...
        if ($write['mb_id'] && $write['mb_id'] === $member['mb_id']) {
            ...
            if ($config['cf_use_point'] && $board['bo_read_point'] && $member['mb_point'] + $board['bo_read_point'] < 0)
                alert(_t('보유하신 포인트').'('.number_format($member['mb_point']).')'._t('가 없거나 모자라서 글읽기').'('.number_format($board['bo_read_point']).')'._t('가 불가합니다.').'\\n\\n'._t('포인트를 모으신 후 다시 글읽기 해 주십시오.'));
    ...
    $g5['title'] = strip_tags(conv_subject(_t($write['wr_subject']), 255))." > "._t($g5['board_title']);
} else {
    if ($member['mb_level'] < $board['bo_list_level']) {
        if ($member['mb_id'])
            alert(_t('목록을 볼 권한이 없습니다.'), G5_URL);
        else
            alert(_t('목록을 볼 권한이 없습니다.').'\\n\\n'._t('회원이시라면 로그인 후 이용해 보십시오.'), './login.php?'.$qstr.'&url='.urlencode(G5_BBS_URL.'/board.php?bo_table='.$bo_table.($qstr?'&amp;':'')));
    }
    ...
    $g5['title'] = _t($g5['board_title']).' '.$page.' '._t('페이지');
}
