...
if (!$is_member) {
    alert_close(_t("사용후기는 회원만 작성이 가능합니다."));
}
...
if ($w == "" || $w == "u") {
    ...
    if (!$is_subject) alert(_t("제목을 입력하여 주십시오."));
    if (!$is_content) alert(_t("내용을 입력하여 주십시오."));
}
...
if ($w == "")
{
    ...
    if ($row['max_is_id'] && $row['max_is_id'] == $max_is_id)
        alert(_t("같은 상품에 대하여 계속해서 평가하실 수 없습니다."));
    */
    ...
    if ($default['de_item_use_use']) {
        $alert_msg = _t("평가하신 글은 관리자가 확인한 후에 출력됩니다.");
    }  else {
        $alert_msg = _t("사용후기가 등록 되었습니다.");
    }
}
else if ($w == "u")
{
    ...
    if ($row['is_password'] != $is_password)
        alert(_t("비밀번호가 틀리므로 수정하실 수 없습니다."));
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        ...
        $row = sql_fetch($sql);
        if (!$row['cnt'])
            alert(_t("자신의 사용후기만 삭제하실 수 있습니다."));
    }
