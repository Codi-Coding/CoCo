...
if($od['od_cancel_price'] > 0 || $ct['od_count1'] != $ct['od_count2']) {
    alert(_t("취소할 수 있는 주문이 아닙니다."), G5_SHOP_URL."/orderinquiryview.php?od_id=$od_id&amp;uid=$uid");
}
...
if($od['od_tno']) {
    switch($od['od_pg']) {
        ...
        case 'inicis':
            ...
            $cancel_msg = iconv_euckr(_t('주문자 본인 취소-').$cancel_memo);
            ...
            if($res_cd != '00') {
                alert(iconv_utf8($res_msg).' '._t('코드').' : '.$res_cd);
            }
            break;
        default:
            ...
            $_POST['mod_desc'] = iconv("utf-8", "euc-kr", _t('주문자 본인 취소-').$cancel_memo);
...
$sql = " update {$g5['g5_shop_order_table']}
            set od_send_cost = '0',
                ...
                od_send_coupon = '0',
                od_status = '취소',
                od_shop_memo = concat(od_shop_memo,\"\\n"._t("주문자 본인 직접 취소")." - ".G5_TIME_YMDHIS." ("._t("취소이유")." : {$cancel_memo})\")
            where od_id = '$od_id' ";
sql_query($sql);
