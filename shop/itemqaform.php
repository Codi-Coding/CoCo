...
if (!$is_member) {
    alert_close(_t("상품문의는 회원만 작성 가능합니다."));
}
...
if ($w == "u")
{
    ...
    if (!$qa) {
        alert_close(_t("상품문의 정보가 없습니다."));
    }

    $it_id    = $qa['it_id'];

    if (!$is_admin && $qa['mb_id'] != $member['mb_id']) {
        alert_close(_t("자신의 상품문의만 수정이 가능합니다."));
    }
...
if(!file_exists($itemqaform_skin)) {
    echo str_replace(G5_PATH.'/', '', $itemqaform_skin).' '._t('스킨 파일이 존재하지 않습니다.');
} else {
