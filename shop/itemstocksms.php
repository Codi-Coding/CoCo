...
<div id="sit_sms_new" class="new_win">
    ...
    <div class="form_01 new_win_con">
        <ul>
            <li class="prd_name">
                <?php echo _t($it['it_name']); ?>
            </li>
            <li>
                <label for="ss_hp" class="sound_only"><?php echo _t('휴대폰번호'); ?><strong> <?php echo _t('필수'); ?></strong></label>
                <input type="text" name="ss_hp" value="<?php echo $member['mb_hp']; ?>" id="ss_hp" required class="required frm_input full_input">
            </li>
            <li>
                <strong><?php echo _t('개인정보처리방침안내'); ?></strong>
                <textarea readonly><?php echo get_text($config['cf_privacy']) ?></textarea>
            </li>
        </ul>
        
        <div id="sms_agree" class="win_desc">
            <label for="agree"><?php echo _t('개인정보처리방침안내의 내용에 동의합니다.'); ?></label>
            <input type="checkbox" name="agree" value="1" id="agree">
        </div>
        <div class="win_btn">
            <input type="submit" value="<?php echo _t('확인'); ?>" class="btn_submit">
            <button type="button" onclick="window.close();" class="btn_close"><?php echo _t('닫기'); ?></button>
        </div>
    </div>
    </form>
</div>
...
{
    if(!f.agree.checked) {
        alert("<?php echo _t('개인정보처리방침안내에 동의해 주십시오.'); ?>");
        return false;
    }

    if(confirm("<?php echo _t('재입고SMS 알림 요청을 등록하시겠습니까?'); ?>")) {
        return true;
    } else {
        window.close();
        return false;
