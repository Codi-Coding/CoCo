...
if (!$is_member) {
    alert_close(_t("사용후기는 회원만 작성 가능합니다."));
}
...
if ($w == "") {
    ...
    if (!$is_admin && $use['mb_id'] != $member['mb_id']) {
        alert_close(_t("자신의 사용후기만 수정이 가능합니다."));
    }
}
...
if(!file_exists($itemuseform_skin)) {
    echo str_replace(G5_PATH.'/', '', $itemuseform_skin).' '._t('스킨 파일이 존재하지 않습니다.');
} else {
