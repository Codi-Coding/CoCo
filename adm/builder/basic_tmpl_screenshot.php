...
if($v == '1')
    ...
    $arr = array($config2w_def['cf_templete']);

for ($i = 0; $i < count($arr); $i++) {
    if($arr[$i] == '.' or $arr[$i] == '..' or is_file("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}")) continue;
    if(preg_match('/^w3_z_/', $arr[$i]) && !preg_match('/_gb$/', $arr[$i])) continue; /// skip sample templates 
?>
<tr class='ht2'>
    <td valign=top style="padding:10px 5px 5px 5px">
    <?php if(file_exists("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/sc.png")) { ?>
    <a href="<?php echo "$g5[url]/$g5[tmpl_dir]/{$arr[$i]}/sc.png" ?>" target="_blank">
    <img src="<?php echo "$g5[url]/$g5[tmpl_dir]/{$arr[$i]}/sc.png" ?>" width=150 height=105 style="margin-left:0px; border:1px solid #dddddd; overflow:hidden">
    </a>
    <?php } else if(file_exists("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/screenshot.png")) { ?>
    <a href="<?php echo "$g5[url]/$g5[tmpl_dir]/{$arr[$i]}/screenshot.png" ?>" target="_blank">
    <img src="<?php echo "$g5[url]/$g5[tmpl_dir]/{$arr[$i]}/screenshot.png" ?>" width=150 height=105 style="margin-left:0px; border:1px solid #dddddd; overflow:hidden">
    </a>
    <?php } else { ?>
    ...
    <?php if(file_exists("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/sc.html")) { ?>
    <?php include "$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/sc.html"; ?>
    <?php } else if(file_exists("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/readme.txt")) { ?>
    <?php
        $file = file("$g5[path]/$g5[tmpl_dir]/{$arr[$i]}/readme.txt");
