...
<tr class='ht2'>
    ...
    <td>
    <div style="float:right">공통 로고: <?php echo($config2w_m['cf_use_common_logo'])?'이용':'이용하지 않음'?></div>
    <select name=cf_use_common_logo>
<?php
     $true_selected  = '';
     $false_selected = '';
     if($config2w_m['cf_use_common_logo']) $true_selected = 'selected';
...
<tr class='ht2'>
    ...
    <td>
    <div style="float:right">공통 메뉴: <?php echo ($config2w_m['cf_use_common_menu'])?'이용':'이용하지 않음'?></div>
    <select name=cf_use_common_menu>
<?php
     $true_selected  = '';
     $false_selected = '';
     if($config2w_m['cf_use_common_menu']) $true_selected = 'selected';
     else $false_selected = 'selected';
     echo "    <option value='1' $true_selected>이용</option>\n";
     echo "    <option value='0' $false_selected>이용하지 않음</option>\n";
?>    </select>
...
<tr class='ht2'>
    ...
    <td>
    <div style="float:right">공통 주소: <?php echo ($config2w_m['cf_use_common_addr'])?'이용':'이용하지 않음'?></div>
    <select name=cf_use_common_addr>
<?php
     $true_selected  = '';
     $false_selected = '';
     if($config2w_m['cf_use_common_addr']) $true_selected = 'selected';
     else $false_selected = 'selected';
     echo "    <option value='1' $true_selected>이용</option>\n";
     echo "    <option value='0' $false_selected>이용하지 않음</option>\n";
?>    </select>
