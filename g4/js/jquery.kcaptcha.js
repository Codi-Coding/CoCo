...
$(function() {
    $('#kcaptcha_image').bind('click', function() {
        $.ajax({
            type: 'POST',
            url: g5_url+'/'+g5_bbs+'/kcaptcha_session.php',
            cache: false,
            async: false,
            success: function(text) {
                $('#kcaptcha_image').attr('src', g5_url+'/'+g5_bbs+'/kcaptcha_image.php?t=' + (new Date).getTime());
            }
        });
    })
    .css('cursor', 'pointer')
...
function check_kcaptcha(input_key)
{
    if (typeof(input_key) != 'undefined') {
        ...
        $.ajax({
            type: 'POST',
            url: g5_url+'/'+g5_bbs+'/kcaptcha_result.php',
            data: {
