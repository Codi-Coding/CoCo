    ...
    function run() {
        ...
        if ($this->query) {
            ...
            if ($this->event) {
                ...
                if($g5['is_db_trans'] && file_exists(G5_PATH.'/locale/include/shop.func.item_list.run.inc.php')) {
                    include G5_PATH.'/locale/include/shop.func.item_list.run.inc.php';
                } else {
                    $sql_select = " select {$this->fields} ";
        ...
        if ($this->list_skin == "") {
            return $this->count._t("번")." item_list() "._t("의 스킨파일이 지정되지 않았습니다.");
...
// 상품 이미지를 얻는다
function get_it_image($it_id, $width, $height=0, $anchor=false, $img_id='', $img_alt='', $is_crop=true) /* 굿빌더 is_crop 변경 */
{
    global $g5;
...
function display_price($price, $tel_inq=false)
{
    ...
    if ($tel_inq)
        ...
        $price = number_format($price, 0)._t('원');
    else
        $price = number_format(_cc($price), $g5['decimal_point'])._t('원');
...
function get_price($it)
{
    ...
    if ($it['it_tel_inq']) return _t('전화문의');
...
{
    ///return number_format($point, 0)._t('점');
    return number_format(_cc($point), $g5['decimal_point'])._t('점');
}
...
function display_type($type, $list_skin='', $list_mod='', $list_row='', $img_width='', $img_height='', $ca_id='')
{
    ...
    */
    ...
    if (!file_exists($file)) {
        return G5_SHOP_SKIN_URL.'/'.$list_skin.' '._t('파일을 찾을 수 없습니다.');
    } else {
...
function email_content($str)
{
    ...
    $s = "";
    $s .= "<html><head><meta http-equiv=\"content-type\" content=\"text/html; charset={$g5['charset']}\"><title>"._t("메일")."</title>\n";
    $s .= "<body>\n";
    $s .= $str;
    $s .= "</body>\n";
    $s .= "</html>";
    ...
    if($subj_count > 1) {
        ...
        for($i=0; $i<$subj_count; $i++) {
            ...
            if($opt_count) {
                ...
                $select .= '<option value="">'._t('선택').'</option>'.PHP_EOL;
                for($k=0; $k<$opt_count; $k++) {
                    $opt_val = $opt[$k];
                    if(strlen($opt_val)) {
                        $select .= '<option value="'.$opt_val.'">'._t($opt_val).'</option>'.PHP_EOL;
                    }
                }
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            if($row['io_price'] >= 0)
                $price = '&nbsp;&nbsp;+ '.number_format($row['io_price'])._t('원');
            else
                $price = '&nbsp;&nbsp; '.number_format($row['io_price'])._t('원');
...
function print_item_options($it_id, $cart_id)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if($row['io_price'] >= 0)
            $price_plus = '+';
        $str .= '<li>'.get_text(_t($row['ct_option'])).' '.$row['ct_qty']._t('개').' ('.$price_plus.display_price($row['io_price']).')</li>'.PHP_EOL;
    }
...
function get_yn($val, $case='')
{
    switch ($case) {
        ...
        default :  $result = ($val > 0) ? _t('예') : _t('아니오');
    }
    return $result;
}
...
{
    $str = '<option value="">'._t('선택').'</option>'.PHP_EOL;
...
function alert_opener($msg='', $url='')
{
    ...
    if (!$msg) $msg = _t('올바른 방법으로 이용해 주십시오.');
...
function relation_item($it_id, $width, $height, $rows=3)
{
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        if($i == 0) {
            $str .= '<span class="sound_only">'._t('관련 상품 시작').'</span>';
    ...
    if($i > 0)
        $str .= '</ul><span class="sound_only">'._t('관련 상품 끝').'</span>';

    return $str;
}

if(defined('G5_USE_OLD_CODE') && G5_USE_OLD_CODE) {
// 상품이미지에 유형 아이콘 출력
function item_icon($it)
{
    ...
    if (is_soldout($it['it_id']))
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_soldout.gif" alt="'._t('품절').'">';

    if ($it['it_type1'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_hit.gif" alt="'._t('히트상품').'">';

    if ($it['it_type2'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_rec.gif" alt="'._t('추천상품').'">';

    if ($it['it_type3'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_new.gif" alt="'._t('최신상품').'">';

    if ($it['it_type4'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_best.gif" alt="'._t('인기상품').'">';

    if ($it['it_type5'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_discount.gif" alt="'._t('할인상품').'">';
    ...
    $row = sql_fetch($sql);
    if($row['cnt'])
        $icon .= '<img src="'.G5_SHOP_URL.'/img/icon_cp.gif" alt="'._t('쿠폰상품').'">';
...
function item_icon($it)
{
    ...
    if ($it['it_type1'])
        $icon .= '<span class="shop_icon shop_icon_1">'._t('히트').'</span>';

    if ($it['it_type2'])
        $icon .= '<span class="shop_icon shop_icon_2">'._t('추천').'</span>';

    if ($it['it_type3'])
        $icon .= '<span class="shop_icon shop_icon_3">'._t('최신').'</span>';

    if ($it['it_type4'])
        $icon .= '<span class="shop_icon shop_icon_4">'._t('BEST').'</span>';

    if ($it['it_type5'])
        $icon .= '<span class="shop_icon shop_icon_5">'._t('할인').'</span>';
    ...
    $row = sql_fetch($sql);
    if($row['cnt'])
        $icon .= '<span class="shop_icon shop_icon_coupon">'._t('쿠폰').'</span>';

    // 품절
    if (is_soldout($it['it_id']))
        $icon .= '<br><span class="shop_icon_soldout">'._t('Sold Out').'</span>';
...
function get_sns_share_link($sns, $url, $title, $img)
{
    ...
    switch($sns) {
        case 'facebook':
            $str = '<a href="https://www.facebook.com/sharer/sharer.php?u='.urlencode($url).'&amp;p='.urlencode($title).'" class="share-facebook" target="_blank"><img src="'.$img.'" alt="'._t('페이스북에 공유').'"></a>';
            break;
        case 'twitter':
            $str = '<a href="https://twitter.com/share?url='.urlencode($url).'&amp;text='.urlencode($title).'" class="share-twitter" target="_blank"><img src="'.$img.'" alt="'._t('트위터에 공유').'"></a>';
            break;
        case 'googleplus':
            $str = '<a href="https://plus.google.com/share?url='.urlencode($url).'" class="share-googleplus" target="_blank"><img src="'.$img.'" alt="'._t('구글플러스에 공유').'"></a>';
            break;
        case 'kakaotalk':
            if($config['cf_kakao_js_apikey'])
                $str = '<a href="javascript:kakaolink_send(\''.str_replace('+', ' ', urlencode($title)).'\', \''.urlencode($url).'\');" class="share-kakaotalk"><img src="'.$img.'" alt="'._t('카카오톡 링크보내기').'"></a>';
            break;
    }
...
function check_itemuse_write($it_id, $mb_id, $close=true)
{
    ...
    if(!$is_admin && $default['de_item_use_write'])
    {
        ...
        {
            if($close)
                alert_close(_t('사용후기는 주문이 완료된 경우에만 작성하실 수 있습니다.'));
            else
                alert(_t('사용후기는 주문하신 상품의 상태가 완료인 경우에만 작성하실 수 있습니다.'));
        }
    }
}
        ...
        case 'item':
            ...
            for($i=0; $i<3; $i++) {
                ...
                if($row['ca_cert_use'] && !$member['mb_certify']) {
                    if($member['mb_id'])
                        $msg = _t('회원정보 수정에서 본인확인 후 이용해 주십시오.');
                    else
                        $msg = _t('본인확인된 로그인 회원만 이용할 수 있습니다.');
                ...
                if($row['ca_adult_use'] && !$member['mb_adult']) {
                    if($member['mb_id'])
                        $msg = _t('본인확인으로 성인인증된 회원만 이용할 수 있습니다.').'\\n'._t('회원정보 수정에서 본인확인을 해주십시오.');
                    else
                        $msg = _t('본인확인으로 성인인증된 회원만 이용할 수 있습니다.');
...
if (!function_exists('make_order_field')) {
// 임시주문 데이터로 주문 필드 생성
