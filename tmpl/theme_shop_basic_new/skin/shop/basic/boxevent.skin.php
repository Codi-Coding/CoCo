...
<div id="sev">
    ...
    for ($i=0; $row=sql_fetch_array($hresult); $i++)
    {
        ...
        if (file_exists($event_img)) { // 이벤트 이미지가 있다면 이미지 출력
            echo '<a href="'.$href.'" class="sev_img "><img src="'.G5_DATA_URL.'/event/'.$row['ev_id'].'_m" alt="'._t($row['ev_subject']).'"></a>'.PHP_EOL;
        } else { // 없다면 텍스트 출력
        ...
        for($k=1; $row2=sql_fetch_array($result2); $k++) {
            ...
            echo '<span class="ev_prd_img">'.get_it_image($row2['it_id'], 110, 110, get_text(_t($row2['it_name']))).'</span>'.PHP_EOL;
            echo '<div class="ev_txt_wr"><a href="'.$item_href.'" class="ev_prd_tit">'.get_text(cut_str(_t($row2['it_name']), 30)).'</a>'.PHP_EOL;
        ...
        if($k == 1) {
            echo '<ul class="ev_prd">'.PHP_EOL;
            echo '<li class="no_prd">'._t('등록된 상품이 없습니다.').'</li>'.PHP_EOL;
            echo '</ul>'.PHP_EOL;
        }
        echo '</div></li>'.PHP_EOL;
    ...
    if ($i==0)
        echo '<li id="sev_empty">'._t('이벤트 없음').'</li>'.PHP_EOL;
    ?>
    </ul>
</div>
<?php
