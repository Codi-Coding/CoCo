...
<div id="sqa_sch">
    ...
    <div class="sch_wr">
    <label for="sfl" class="sound_only"><?php echo _t('검색항목'); ?><strong class="sound_only"> <?php echo _t('필수'); ?></strong></label>
    <select name="sfl" id="sfl" required>
        <option value=""><?php echo _t('선택'); ?></option>
        <option value="b.it_name"    <?php echo get_selected($sfl, "b.it_name", true); ?>><?php echo _t('상품명'); ?></option>
        <option value="a.it_id"      <?php echo get_selected($sfl, "a.it_id"); ?>><?php echo _t('상품코드'); ?></option>
        <option value="a.iq_subject" <?php echo get_selected($sfl, "a.iq_subject"); ?>><?php echo _t('문의제목'); ?></option>
        <option value="a.iq_question"<?php echo get_selected($sfl, "a.iq_question"); ?>><?php echo _t('문의내용'); ?></option>
        <option value="a.iq_name"    <?php echo get_selected($sfl, "a.iq_name"); ?>><?php echo _t('작성자명'); ?></option>
...
<div id="sqa">
    ...
    <li>
        ...
        <div class="sqa_img">
            <a href="<?php echo $it_href; ?>">
                ...
                <span><?php echo _t($row['it_name']); ?></span>
            </a>
        </div>
        ...
        <section class="sqa_section">
            ...
            <dl class="sqa_dl">
                <dt class="sound_only"><?php echo _t('작성자'); ?></dt>
                <dd><i class="fa fa-user" aria-hidden="true"></i> <?php echo $row['iq_name']; ?></dd>
                <dt class="sound_only"><?php echo _t('작성일'); ?></dt>
            ...
            <div id="sqa_con_<?php echo $i; ?>" class="sqa_con" style="display:none;">
                <div class="sit_qa_qaq">
                    <strong class="sound_only"><?php echo _t('문의내용'); ?></strong>
                    <span class="qa_alp">Q</span>
                    <?php echo $iq_question; // 상품 문의 내용 ?>
                </div>
                <?php if(!$is_secret) { ?>
                <div class="sit_qa_qaa">
                    <strong class="sound_only"><?php echo _t('답변'); ?></strong>
            ...
            <div class="sqa_con_btn"><button class="sqa_con_<?php echo $i; ?>"><?php echo _t('내용보기'); ?> <i class="fa fa-caret-down" aria-hidden="true"></i></button></div>
        </section>
    ...
    if ($i > 0) echo '</ol>';
    if ($i == 0) echo '<p id="sqa_empty">'._t('자료가 없습니다.').'</p>';
    ?>
</div>
...
$(function(){
    ...
    $(".sqa_con_btn button").click(function(){
        ...
        if($con.is(":visible")) {
            $con.slideUp();
            $(this).html("<?php echo _t('내용보기'); ?> <i class=\"fa fa-caret-down\" aria-hidden=\"true\"></i>");
        } else {
            $(".sps_con_btn button").html("<?php echo _t('내용보기'); ?> <i class=\"fa fa-caret-down\" aria-hidden=\"true\"></i>");
            ...
            $(this).html("<?php echo _t('내용닫기'); ?> <i class=\"fa fa-caret-up\" aria-hidden=\"true\"></i>");
        }
    });
});
</script>
