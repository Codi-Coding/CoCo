...
<h2 id="container_title"><?php echo $g5['title'] ?><span class="sound_only"> <?php echo _t('목록'); ?></span></h2><!-- w3 -->
...
<fieldset id="new_sch">
    ...
    <label for="view" class="sound_only"><?php echo _t('검색대상'); ?></label>
    <select name="view" id="view">
        <option value=""><?php echo _t('전체게시물'); ?>
        <option value="w"><?php echo _t('원글만'); ?>
        <option value="c"><?php echo _t('코멘트만'); ?>
    </select>
    <label for="mb_id" class="sound_only"><?php echo _t('검색어'); ?><strong class="sound_only"> <?php echo _t('필수'); ?></strong></label>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <?php if ($is_admin) { ?>
        <th scope="col">
            <label for="all_chk" class="sound_only"><?php echo _t('목록 전체'); ?></label>
            <input type="checkbox" id="all_chk">
        </th>
        <?php } ?>
        <th scope="col"><?php echo _t('그룹'); ?></th>
        <th scope="col"><?php echo _t('게시판'); ?></th>
        <th scope="col"><?php echo _t('제목'); ?></th>
        <th scope="col"><?php echo _t('이름'); ?></th>
        <th scope="col"><?php echo _t('일시'); ?></th>
    </tr>
    </thead>
    <tbody>
    <?php
    for ($i=0; $i<count($list); $i++)
    {
        ...
        $gr_subject = cut_str(_t($list[$i]['gr_subject']), 20);
        $bo_subject = cut_str(_t($list[$i]['bo_subject']), 20);
        $wr_subject = get_text(cut_str(_t($list[$i]['wr_subject']), 80));
    ?>
    <tr>
        <?php if ($is_admin) { ?>
        <td class="td_chk">
            <label for="chk_bn_id_<?php echo $i; ?>" class="sound_only"><?php echo $num?> <?php echo _t('번'); ?></label>
    ...
    <?php if ($i == 0)
        echo '<tr><td colspan="'.$colspan.'" class="empty_table">'._t('게시물이 없습니다.').'</td></tr>';
    ?>
    </tbody>
    </table>
</div>
...
<div class="sir_bw02 sir_bw">
    <input type="submit" onclick="document.pressed=this.value" value="<?php echo _t('선택삭제'); ?>" class="btn_submit">
</div>
<?php } ?>
</form>
...
function fnew_submit(f)
{
    ...
    if (!confirm("<?php echo _t('선택한 게시물을 정말'); ?> "+document.pressed+" <?php echo _t('하시겠습니까?').'\n\n'._t('한번 삭제한 자료는 복구할 수 없습니다'); ?>")) {
        return false;
    }
