...
{
    if ($i==0) echo '<section id="sbn_idx" class="sbn">'.PHP_EOL.'<h2>'._t('컨텐츠몰 배너').'</h2>'.PHP_EOL.'<ul>'.PHP_EOL;
    //print_r2($row);
    // 테두리 있는지
    ...
    if (file_exists($bimg))
    {
        ...
        echo '<img src="'.G5_DATA_URL.'/cmbanner/'.$row['bn_id'].'" width="'.$size[0].'" alt="'.$row['bn_alt'].'" class="'.$bn_border.'">';
        if($row['bn_url'])
            echo '</a>'.PHP_EOL;
        echo '</li>'.PHP_EOL;
