...
<div id="poll_result" class="new_win">
    ...
    <section id="poll_result_list">
        <h2><?php echo _t($po_subject) ?> <?php echo _t('결과'); ?></h2>

        <dl>
            <dt><span><?php echo _t('전체'); ?> <?php echo $nf_total_po_cnt ?><?php echo _t('표'); ?></span></dt>
            <dd>
                <ol>
                <?php for ($i=1; $i<=count($list); $i++) {  ?>
                    <li>
                        <p>
                            <?php echo _t($list[$i]['content']) ?>
                            <strong><?php echo $list[$i]['cnt'] ?> <?php echo _t('표'); ?></strong>
                            <span><?php echo number_format($list[$i]['rate'], 1) ?> <?php echo _t('퍼센트'); ?></span>
                        </p>
    ...
    <section id="poll_result_cmt">
        ...
        <article>
            <header>
                <h1><?php echo $list2[$i]['pc_name'] ?><span class="sound_only"><?php echo _t('님의 의견'); ?></span></h1>
                <?php echo $list2[$i]['name'] ?>
                <span class="poll_datetime"><?php echo $list2[$i]['datetime'] ?></span>
            </header>
            <p>
                <?php echo _t($list2[$i]['idea']) ?>
            </p>
            <footer>
                <span class="poll_cmt_del"><?php if ($list2[$i]['del']) { echo $list2[$i]['del']._t("삭제")."</a>"; }  ?></span>
            </footer>
        </article>
        <?php }  ?>
        ...
        <div class="tbl_frm01 tbl_wrap">
            ...
            <?php if ($is_guest) {  ?>
            <tr>
                <th scope="row"><label for="pc_name"><?php echo _t('이름'); ?><strong class="sound_only"><?php echo _t('필수'); ?></strong></label></th>
                <td><input type="text" name="pc_name" id="pc_name" required class="frm_input required" size="10"></td>
            </tr>
            <?php }  ?>
            <tr>
                <th scope="row"><label for="pc_idea"><?php echo _t('의견'); ?><strong class="sound_only"><?php echo _t('필수'); ?></strong></label></th>
    ...
    <aside id="poll_result_oth">
        ...
        <ul>
            ...
            <li><a href="./poll_result.php?po_id=<?php echo $list3[$i]['po_id'] ?>&amp;skin_dir=<?php echo urlencode($skin_dir); ?>">[<?php echo $list3[$i]['date'] ?>] <?php echo _t($list3[$i]['subject']) ?></a></li>
            <?php }  ?>
        </ul>
    </aside>
    <!-- } 설문조사 다른 결과 보기 끝 -->
