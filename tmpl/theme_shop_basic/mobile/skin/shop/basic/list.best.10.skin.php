...
if($this->total_count > 0) {
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        if($i > 0 && ($i % $this->list_mod == 0)) {
            ...
            echo '<ul class="sct_best">'.PHP_EOL;
            $k++;
            $slide_btn .= '<button type="button">'.$k._t('번째 리스트').'</button>';
        }
        ...
        if ($this->view_it_img) {
            echo get_it_image($row['it_id'], $this->img_width, $this->img_height, '', '', stripslashes(_t($row['it_name']))).PHP_EOL;
        }

        if ($this->href) {
            echo '</a><span class="best_icon">'._t('베스트상품').'</span></div>'.PHP_EOL;
        }
        ...
        if ($this->view_it_name) {
            echo stripslashes(_t($row['it_name'])).PHP_EOL;
        }
