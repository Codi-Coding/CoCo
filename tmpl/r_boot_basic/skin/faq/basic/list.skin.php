...
<nav id="bo_cate">
    ...
    <ul id="bo_cate_ul">
        ...
        ?>
        <li><a href="<?php echo $category_href;?>?fm_id=<?php echo $v['fm_id'];?>" <?php echo $category_option;?> ><?php echo $category_msg._t($v['fm_subject']);?></a></li>
        <?php
        }
        ?>
    </ul>
...
<div id="faq_wrap" class="faq_<?php echo $fm_id; ?>">
    ...
    ?>
    <section id="faq_con">
        <h2><?php echo _t($g5['title']); ?> <?php echo _t('목록'); ?></h2>
        <ol>
            <?php
            ...
            ?>
            <li>
                <h3><a href="#none" onclick="return faq_open(this);"><?php echo conv_content(_t($v['fa_subject']), 1); ?></a></h3>
                <div class="con_inner">
                    <?php echo conv_content(_t($v['fa_content']), 1); ?>
                    <div class="con_closer"><button type="button" class="closer_btn"><?php echo _t('닫기'); ?></button></div>
                </div>
            </li>
            <?php
            }
    ...
    <?php
        ...
        if($stx){
            echo '<p class="empty_list">'._t('검색된 게시물이 없습니다.').'</p>';
        } else {
            echo '<div class="empty_list">'._t('등록된 FAQ가 없습니다.');
            if($is_admin)
...
<fieldset id="faq_sch">
    <legend><?php echo _t('FAQ 검색'); ?></legend>
