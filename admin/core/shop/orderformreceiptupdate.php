...
if($cart_status) {
	...
	/**
	 * 무통장 주문의 입금확인 시, 공유회원 아이디가 없을 경우
	 */
	if ($od['od_settle_case'] == '무통장' && $od['od_sit_mbid'] == '') {
		
		/**
		 * 장바구니에서 가장 최근의 공유판매 상품에서 공유판매회원 정보를 추출
		 */
		$sinfo = sql_fetch("select it_id, sh_mb_id from {$g5['g5_shop_cart_table']} where sh_mb_id != '' and od_id = '$od_id' order by ct_select_time asc limit 1");
		...
		if ($sinfo['sh_mb_id']) {
			/**
			 * 자신이 공유판매한 제품에 대해서는 예외처리
			 */
			if ($od['mb_id'] != $sinfo['sh_mb_id']) {
				/**
				 * 구매자의 추천인이 있는지 체크
				 */
				$minfo = sql_fetch("select mb_recommend from {$g5['member_table']} where mb_id = '{$od['mb_id']}' ");
				if (!$minfo['mb_recommend']) {	
					/**
					 * 공유판매 공유회원을 추천인 아이디로 등록 쿼리
					 */			
					$recommend_sql = "update {$g5['member_table']} set mb_recommend = '{$sinfo['sh_mb_id']}' where mb_id = '{$od['mb_id']}' ";
					$od_sit_mbid = $sinfo['sh_mb_id'];
				} else {
					$od_sit_mbid = $minfo['mb_recommend'];
				}
				
				/**
				 * 관리자일 경우는 예외처리
				 */
				if ($od['mb_id'] != $config['cf_admin'] && $sinfo['sh_mb_id'] != $config['cf_admin']) {
					$od_sit_mbid = '';
				} else {
					 ...
					 * 추천인 등록 실행
					 */
					sql_query($recommend_sql);
					
					/**
					 * 이 주문에 대한 공유판매 회원의 아이디 등록
					 */
					if ($od_sit_mbid) {
						sql_query("update {$g5['g5_shop_order_table']} set od_sit_mbid = '{$od_sit_mbid}' where od_id = '{$od_id}' ");
					}
				}
			}
		}
...
alert("주문정보를 수정하였습니다.", EYOOM_ADMIN_URL . "/?dir=shop&amp;pid=orderform&amp;od_id={$od_id}&amp;{$qstr}");
?>
