...
function gugun_make(sido)
{
    ...
    gugun.options[0].text  = "- "+g5_msg_select_si_gun_gu+" -";
    ...
    for (i=0; i<sojae.length; i++) {
        gugun.options[i+1].value = (sojae[i] == g5_msg_no_exist) ? "" : sojae[i];
...
function search_call(page)
{
    ...
    $.getJSON("//api.poesis.kr/post/search.php?sido="+sido+"&gugun="+gugun+"&q="+q+"&page="+page+"&callback=?",
        function(data) {
            $("#result").empty();
