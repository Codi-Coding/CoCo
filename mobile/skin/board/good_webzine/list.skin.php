...
<h2 id="container_title"><?php echo ($board['bo_mobile_subject'] ? $board['bo_mobile_subject'] : $board['bo_subject']) ?><span class="sound_only"> <?php echo _t('목록'); ?></span></h2>
...
<div id="bo_list<?php if ($is_admin) echo "_admin"; ?>">
    ...
    <div class="tbl_head01 tbl_wrap">
        ...
        for ($i=0; $i<count($list); $i++) {
                        ...
                        if($thumb['src']) {
                            $img_content = '<img src="'.$thumb['src'].'" alt="'.$thumb['alt'].'" width="'.$board['bo_mobile_gallery_width'].'" height="'.$board['bo_mobile_gallery_height'].'" style="float:left; margin-right:10px">';
                        } else {
                            $img_content = '<div style="width:'.$board['bo_mobile_gallery_width'].'px; height:'.$board['bo_mobile_gallery_height'].'px; float:left; margin-right:10px; padding:35px 30px; border:1px solid #ddd; color:#ccc">no image</div>';
                        }
        ?>
        <tr class="<?php if ($list[$i]['is_notice']) echo "bo_notice"; ?>">
            <?php if ($is_checkbox) { ?>
            <td class="td_chk">
                <label for="chk_wr_id_<?php echo $i ?>" class="sound_only"><?php echo _t($list[$i]['subject']) ?></label>
            ...
            <p>
                ...
                ?>
                <a href="<?php echo $list[$i]['ca_name_href'] ?>" class="bo_cate_link"><?php echo _t($list[$i]['ca_name']) ?></a>
                <?php } ?>

                <a href="<?php echo $list[$i]['href'] ?>">
                    <b><?php echo _t($list[$i]['subject']) ?></b>
                ...
                <!--<br/>
                <span class="sort_info"><?php echo _t('작성자'); ?>: <?php echo $list[$i]['name']?></span>
                <span class="sort_info_right"><?php echo _t('조회'); ?>: <?php echo  $list[$i]['wr_hit'];?><?php if ($is_good) { ?>, <?php echo _t('추천'); ?>: <?php echo $list[$i]['wr_good'] ?><?php } ?><?php if ($is_nogood) { ?>, <?php echo _t('비추천'); ?>: <?php echo $list[$i]['wr_nogood'] ?><?php } ?></span>-->
                <br style="clear:right; margin-top:5px"/>
                <a href="<?php echo $list[$i]['href'] ?>">
                <?php echo mb_substr(strip_tags(_t($list[$i]['content'])), 0, $content_len, 'utf-8'); ?>
                <?php if(mb_strlen(strip_tags(_t($list[$i]['content'])), 'utf-8') > $content_len) echo " ....."; ?>
                </a>
            </p>
            </td>
        </tr>
