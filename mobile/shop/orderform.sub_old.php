...
if($g5['is_db_trans'] && file_exists($g5['locale_path'].'/include/ml/mobile/shop'.'/orderform.sub.ml.php')) { include_once $g5['locale_path'].'/include/ml/mobile/shop'.'/orderform.sub.ml.php'; return; }
...
if($local_pg) require_once(G5_MSHOP_PATH.'/settle_'.$default['de_pg_service'].'.inc.php'); ///goodbuilder
...
ob_start();
?>
    <p><?php echo _t('주문하실 상품을 확인하세요.'); ?></p>
    ...
    <ul class="sod_list">
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++)
        {
            ...
            $it_name = $a1 . stripslashes(_t($row['it_name'])) . $a2;
            ...
            if($is_member) {
                ...
                if($cp_count) {
                    $cp_button = '<div class="li_cp"><button type="button" class="cp_btn">'._t('쿠폰적용').'</button></div>';
                    $it_cp_count++;
                }
            }
        ...
        <li class="sod_li">
            ...
            <div class="li_prqty">
                <span class="prqty_price li_prqty_sp"><span><?php echo _t('판매가'); ?> </span><?php echo number_format(_cc($row['ct_price']), $g5['decimal_point']); ?></span>
                <span class="prqty_qty li_prqty_sp"><span><?php echo _t('수량'); ?> </span><?php echo number_format($sum['qty']); ?></span>
                <span class="prqty_sc li_prqty_sp"><span><?php echo _t('배송비'); ?> </span><?php echo number_format(_cc($ct_send_cost), $g5['decimal_point']); ?></span>
            </div>
            <div class="li_total">
                <span class="total_price total_span"><span><?php echo _t('주문금액'); ?> </span><strong><?php echo number_format(_cc($sell_price), $g5['decimal_point']); ?></strong></span>
                <span class="total_point total_span"><span><?php echo _t('적립포인트'); ?> </span><strong><?php echo number_format($sum['point']); ?></strong></span>
            </div>
    ...
    <dl id="sod_bsk_tot">
        <dt class="sod_bsk_sell"><?php echo _t('주문'); ?></dt>
        <dd class="sod_bsk_sell"><strong><?php echo number_format(_cc($tot_sell_price), $g5['decimal_point']); ?> <?php echo _t('원'); ?></strong></dd>
        <?php if($it_cp_count > 0) { ?>
        <dt class="sod_bsk_coupon"><?php echo _t('쿠폰'); ?></dt>
        <dd class="sod_bsk_coupon"><strong id="ct_tot_coupon">0 <?php echo _t('원'); ?></strong></dd>
        <?php } ?>
        <dt class="sod_bsk_dvr"><?php echo _t('배송비'); ?></dt>
        <dd class="sod_bsk_dvr"><strong><?php echo number_format(_cc($send_cost), $g5['decimal_point']); ?> <?php echo _t('원'); ?></strong></dd>
        <dt class="sod_bsk_cnt"><?php echo _t('총계'); ?></dt>
        <dd class="sod_bsk_cnt">
            <?php $tot_price = $tot_sell_price + $send_cost; // 총계 = 주문상품금액합계 + 배송비 ?>
            <strong id="ct_tot_price"><?php echo number_format(_cc($tot_price), $g5['decimal_point']); ?> <?php echo _t('원'); ?></strong>
        </dd>
...
if($local_pg) require_once(G5_MSHOP_PATH.'/'.$default['de_pg_service'].'/orderform.1.php'); ///goodbuilder
...
<div id="sod_frm">
    ...
    <section id="sod_frm_orderer" >
        ...
        <div class="odf_tbl">
            ...
            <tr>
                <th scope="row"><label for="od_pwd"><?php echo _t('비밀번호'); ?><strong class="sound_only"> <?php echo _t('필수'); ?></strong></label></th>
                <td>
                    <input type="password" name="od_pwd" id="od_pwd" required class="frm_input required" maxlength="20">
                    <?php echo _t('영,숫자 3~20자 (주문서 조회시 필요)'); ?>
                </td>
            </tr>
            <?php } ?>
            ...
            <tr>
                ...
                <td>
                    ...
                    <button type="button" class="btn_frmline" onclick="win_zip('forderform', 'od_zip', 'od_addr1', 'od_addr2', 'od_addr3', 'od_addr_jibeon');"><?php echo _t('주소 검색'); ?></button><br>
                    <label for="od_addr1" class="sound_only"><?php echo _t('기본주소'); ?><strong class="sound_only"> <?php echo _t('필수'); ?></strong></label>
                    ...
                    <label for="od_addr3" class="sound_only"><?php echo _t('참고항목'); ?></label>
            ...
            <tr>
                <th scope="row"><label for="od_email">E-mail<strong class="sound_only"> <?php echo _t('필수'); ?></strong></label></th>
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="text" name="od_hope_date" value="" id="od_hope_date" required class="frm_input required" size="11" maxlength="10" readonly> <?php echo _t('이후로 배송 바랍니다.'); ?>
                </td>
            </tr>
            <?php } ?>
            </tbody>
    ...
    <section id="sod_frm_taker">
        ...
        <div class="odf_tbl">
            ...
            if($is_member) {
                ...
                $addr_list .='<a href="'.G5_SHOP_URL.'/orderaddress.php" id="order_address">'._t('배송지목록').'</a>';
            } else {
                // 주문자와 동일
            ...
            <tr>
                ...
                <td id="sod_frm_addr">
                    ...
                    <button type="button" class="btn_frmline" onclick="win_zip('forderform', 'od_b_zip', 'od_b_addr1', 'od_b_addr2', 'od_b_addr3', 'od_b_addr_jibeon');"><?php echo _t('주소 검색'); ?></button><br>
                    <label for="od_b_addr1" class="sound_only"><?php echo _t('기본주소'); ?><strong class="sound_only"> <?php echo _t('필수'); ?></strong></label>
                    ...
                    <label for="od_b_addr3" class="sound_only"><?php echo _t('참고항목'); ?></label>
                    <input type="text" name="od_b_addr3" id="od_b_addr3" class="frm_input frm_address" readonly="readonly">
                    <input type="hidden" name="od_b_addr_jibeon" value="">
                </td>
            </tr>
            <tr>
                <th scope="row"><label for="od_memo"><?php echo _t('전하실 말씀'); ?></label></th>
    ...
    <section id="sod_frm_pay">
        ...
        <div class="odf_tbl">
            ...
            <?php if($oc_cnt > 0) { ?>
            <tr>
                <th scope="row"><?php echo _t('주문할인쿠폰'); ?></th>
                <td>
            ...
            <tr>
                <th scope="row"><?php echo _t('주문할인금액'); ?></th>
                <td><span id="od_cp_price">0</span><?php echo _t('원'); ?></td>
            </tr>
            <?php } ?>
            <?php if($sc_cnt > 0) { ?>
            <tr>
                <th scope="row"><?php echo _t('배송비할인쿠폰'); ?></th>
                <td>
            ...
            <tr>
                <th scope="row"><?php echo _t('배송비할인금액'); ?></th>
                <td><span id="sc_cp_price">0</span><?php echo _t('원'); ?></td>
            </tr>
            <?php } ?>
            <tr>
                <th><?php echo _t('총 주문금액'); ?></th>
                <td><span id="od_tot_price"><?php echo number_format(_cc($tot_price), $g5['decimal_point']); ?></span><?php echo _t('원'); ?></td>
            </tr>
             <tr>
                <th><?php echo _t('추가배송비'); ?></th>
                <td><span id="od_send_cost2">0</span><?php echo _t('원'); ?> (<?php echo _t('지역에 따라 추가되는 도선료 등의 배송비입니다.'); ?>)</td>
            </tr>
            </tbody>
            </table>
        </div>
        ...
        if ($default['de_bank_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_bank" name="od_settle_case" value="무통장" '.$checked.'> <label for="od_settle_bank">'._t('무통장입금').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_vbank_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_vbank" name="od_settle_case" value="가상계좌" '.$checked.'> <label for="od_settle_vbank">'.$escrow_title._t('가상계좌').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_iche_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_iche" name="od_settle_case" value="계좌이체" '.$checked.'> <label for="od_settle_iche">'.$escrow_title._t('계좌이체').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_hp_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_hp" name="od_settle_case" value="휴대폰" '.$checked.'> <label for="od_settle_hp">'._t('휴대폰').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_card_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_card" name="od_settle_case" value="신용카드" '.$checked.'> <label for="od_settle_card">'._t('신용카드').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if($default['de_samsung_pay_use']) {
            echo '<li><input type="radio" id="od_settle_samsungpay" data-case="samsungpay" name="od_settle_case" value="삼성페이" '.$checked.'> <label for="od_settle_samsungpay" class="samsung_pay">'._t('삼성페이').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_alipay_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_alipay" name="od_settle_case" value="alipay" '.$checked.'> <label for="od_settle_alipay">ALIPAY</label></li>'.PHP_EOL;
        ...
        if ($default['de_anet_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_anet" name="od_settle_case" value="anet" '.$checked.'> <label for="od_settle_anet">Authorize.Net</label></li>'.PHP_EOL;
        ...
        if ($default['de_eximbay_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="od_settle_eximbay" name="od_settle_case" value="eximbay" '.$checked.'> <label for="od_settle_eximbay">Eximbay</label></li>'.PHP_EOL;
        ...
        if ($is_member && $config['cf_use_point'])
        {
            ...
            if ($member['mb_point'] >= $default['de_settle_min_point'])
            {
                ...
                echo '<div id="sod_frm_pt"><input type="hidden" name="max_temp_point" value="'.$temp_point.'">'._t('결제포인트').' : <input type="text" id="od_temp_point" name="od_temp_point" value="0" class="frm_input" size="10">'._t('점').' ('.$point_unit._t('점 단위로 입력하세요.').')</div>';
                echo '<p id="sod_frm_pt_info">'._t('회원님의 보유포인트').'('.display_point($member['mb_point']).')'._t('중').' <strong id="use_max_point">'.display_point($temp_point).'</strong>'._t('까지 사용 가능합니다.').'</p>';
                $multi_settle++;
            }
        }
    ...
    if($local_pg) require_once(G5_MSHOP_PATH.'/'.$default['de_pg_service'].'/orderform.2.php'); ///goodbuilder
    else echo '
    <div id="display_pay_button" class="btn_confirm">
    <input type="submit" value="'._t('주문하기').'" class="btn_submit">
    <a href="'.G5_SHOP_URL.'" class="btn01">'._t('취소').'</a>
    </div>
    ';
    ...
    <div id="show_progress" style="display:none;">
        ...
        <span><?php echo _t('주문완료 중입니다. 잠시만 기다려 주십시오.'); ?></span>
    </div>
    ...
    if ($default['de_escrow_use']) {
        ...
        if($local_pg) require_once(G5_MSHOP_PATH.'/'.$default['de_pg_service'].'/orderform.3.php'); ///goodbuilder
...
$(function() {
    ...
    $(document).on("click", ".cp_apply", function() {
        ...
        if(cp_dup) {
            ...
            if(!confirm(subj+ "<?php echo _t('쿠폰은'); ?> "+it_name+"<?php echo _t('에 사용되었습니다.'); ?>\n"+it_name+"<?php echo _t('의 쿠폰을 취소한 후 적용하시겠습니까?'); ?>")) {
                ...
                $("#cp_frm").remove();
                $cp_dup_el.find(".cp_btn").text("<?php echo _t('쿠폰적용'); ?>").removeClass("cp_mod").focus();
        ...
        $("#cp_frm").remove();
        $cp_btn_el.text("<?php echo _t('쿠폰변경'); ?>").addClass("cp_mod").focus();
        if(!$cp_row_el.find(".cp_cancel").size())
            $cp_btn_el.after("<button type=\"button\" class=\"cp_cancel\"><?php echo _t('쿠폰취소'); ?></button>");
    });
    ...
    $(document).on("click", ".cp_cancel", function() {
        ...
        calculate_total_price();
        $("#cp_frm").remove();
        $(this).closest("li").find(".cp_btn").text("<?php echo _t('쿠폰적용'); ?>").removeClass("cp_mod").focus();
        $(this).remove();
    });
    ...
    $(document).on("click", ".od_cp_apply", function() {
        ...
        $("#od_coupon_frm").remove();
        $("#od_coupon_btn").text("<?php echo _t('쿠폰변경'); ?>").focus();
        if(!$("#od_coupon_cancel").size())
            $("#od_coupon_btn").after("<button type=\"button\" id=\"od_coupon_cancel\" class=\"btn_frmline\"><?php echo _t('쿠폰취소'); ?></button>");
    });
    ...
    $(document).on("click", ".sc_cp_apply", function() {
        ...
        calculate_order_price();
        $("#sc_coupon_frm").remove();
        $("#sc_coupon_btn").text("<?php echo _t('쿠폰변경'); ?>").focus();
        if(!$("#sc_coupon_cancel").size())
            $("#sc_coupon_btn").after("<button type=\"button\" id=\"sc_coupon_cancel\" class=\"btn_frmline\"><?php echo _t('쿠폰취소'); ?></button>");
    });
    ...
    ///goodbuilder
    $("#od_settle_paypal,#od_settle_alipay,#od_settle_anet, #od_settle_eximbay").bind("click", function() {
        $("#settle_bank").hide();
    });
...
function calculate_total_price()
{
    ...
    $("#ct_tot_coupon").text(number_format(String(tot_cp_price))+" <?php echo _t('원'); ?>");
    $("#ct_tot_price").text(number_format(String(tot_sell_price))+" <?php echo _t('원'); ?>");
...
function calculate_order_price()
{
    ...
    if(g5_use_currency_conversion == 1) {
        ...
        $("#od_tot_price").text(number_format2(String(tot_price), 2));
    } else {
        $("#od_tot_price").text(number_format(String(tot_price)));
    }
    <?php if($temp_point > 0 && $is_member) { ?>
...
function calculate_temp_point()
{
    ...
    $("#use_max_point").text("<?php echo _t('최대'); ?> "+number_format(String(temp_point))+"<?php echo _t('점'); ?>");
...
function forderform_check()
{
    ...
    ///goodbuilder
    if(settle_method != "무통장" && settle_method != "paypal" && settle_method != "alipay" && settle_method != "anet" && settle_method != "eximbay" && f.res_cd.value != "0000") {
        alert("<?php echo _t('결제등록요청 후 주문해 주십시오.'); ?>");
        return false;
    }
