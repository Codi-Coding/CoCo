    ...
    <div class="list_01">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            if($row['cp_method'] == 1) {
                ...
                $ca = sql_fetch($sql);
                $cp_target = $ca['ca_name']._t('의 상품할인');
                ...
                $it = sql_fetch($sql);
                $cp_target = $it['it_name'].' '._t('상품할인');
            }

            if($row['cp_type'])
                $cp_price = '<strong>'.$row['cp_price'].'</strong> %';
            else
                $cp_price = '<strong>'.number_format($row['cp_price']).'</strong> '._t('원');
        ...
        if(!$cp_count)
            echo '<li class="empty_list">'._t('사용할 수 있는 쿠폰이 없습니다.').'</li>';
        ?>
        </ul>
    </div>

    <div class="win_btn"><button type="button" onclick="window.close();" class="btn_close"><?php echo _t('창닫기'); ?></button></div>
</div>
