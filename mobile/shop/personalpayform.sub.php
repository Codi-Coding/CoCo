...
<div id="m_pv_sod_frm">
    ...
    <section id="m_sod_frm_orderer">
       ...
       echo '<ul>';
        ...
        if ($default['de_vbank_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="pp_settle_vbank" name="pp_settle_case" value="가상계좌" '.$checked.'> <label for="pp_settle_vbank">'.$escrow_title._t('가상계좌').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_iche_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="pp_settle_iche" name="pp_settle_case" value="계좌이체" '.$checked.'> <label for="pp_settle_iche">'.$escrow_title._t('계좌이체').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_hp_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="pp_settle_hp" name="pp_settle_case" value="휴대폰" '.$checked.'> <label for="pp_settle_hp">'._t('휴대폰').'</label></li>'.PHP_EOL;
            $checked = '';
        }
        ...
        if ($default['de_card_use']) {
            $multi_settle++;
            echo '<li><input type="radio" id="pp_settle_card" name="pp_settle_case" value="신용카드" '.$checked.'> <label for="pp_settle_card">'._t('신용카드').'</label></li>'.PHP_EOL;
            $checked = '';
        }
    ...
    <div id="show_progress" style="display:none;">
        ...
        <span><?php echo _t('결제진행 중입니다. 잠시만 기다려 주십시오.'); ?></span>
    </div>
    </form>
...
function forderform_check()
{
    ...
    if(f.res_cd.value != "0000") {
        alert("<?php echo _t('결제등록요청 후 결제해 주십시오.'); ?>");
        return false;
    }
