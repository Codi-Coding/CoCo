...
if (!$pp['pp_id'] || (md5($pp['pp_id'].$pp['pp_time'].$_SERVER['REMOTE_ADDR']) != get_session('ss_personalpay_uid'))) {
    if( isset($_GET['ini_noti']) && $pp['pp_tno'] ){
        alert(_t("해당 개인결제는 정상적으로 결제되었습니다."), G5_SHOP_URL."/personalpay.php");
    } else {
        alert(_t("조회하실 개인결제 내역이 없습니다."), G5_SHOP_URL."/personalpay.php");
    }
}
        ...
        <section id="sod_fin_pay">
            ...
            <div class="odf_tbl">
                ...
                <tr>
                    ...
                    {
                    ?>
                        <a href="javascript:;" onclick="window.open('<?php echo G5_SHOP_URL; ?>/taxsave.php?tx=personalpay&od_id=<?php echo $pp_id; ?>', 'taxsave', 'width=550,height=400,scrollbars=1,menus=0');"><?php echo _t('현금영수증을 발급하시려면 클릭하십시오.'); ?></a>
                    <?php } ?>
                    </td>
                </tr>
                <?php
