...
if($g5['is_db_trans'] && file_exists($g5['locale_path'].'/include/ml/mobile/shop'.'/item.ml.php')) { include_once $g5['locale_path'].'/include/ml/mobile/shop'.'/item.ml.php'; return; }

if(defined('G5_USE_OLD_CODE') && G5_USE_OLD_CODE) { include_once G5_MSHOP_PATH.'/item_old.php'; return; }
...
$row = sql_fetch($sql);
if ($row['it_id']) {
    $prev_title = _t('이전상품').' <span>'._t($row['it_name']).'</span>';
...
$row = sql_fetch($sql);
if ($row['it_id']) {
    $next_title = _t('다음 상품').' <span>'._t($row['it_name']).'</span>';
