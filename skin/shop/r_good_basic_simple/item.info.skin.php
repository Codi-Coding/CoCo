...
<section id="sit_inf">
    ...
    <colgroup>
        <!--<col class="grid_4">--> <!--굿빌더 수정: 반응형 지원-->
        <col class="grid_3">
        <col>
    </colgroup>
    <tbody>
    ...
    <?php
            ...
            if($is_admin) {
                echo '<p>'._t('상품 정보 고시 정보가 올바르게 저장되지 않았습니다.').'<br>'._t('config.php 파일의 G5_ESCAPE_FUNCTION 설정을 addslashes 로').'<br>'._t('변경하신 후 관리자').' &gt; '._t('상품정보 수정에서 상품 정보를 다시 저장해주세요.').' </p>';
            }
        }
    } //if
    ?>
...
<!-- 배송정보 시작 { -->
<section id="sit_dvr">
    <h2><?php echo _t('배송정보'); ?></h2>
    <?php echo pg_anchor('dvr'); ?>

    <?php echo conv_content(_t($default['de_baesong_content']), 1); ?>
</section>
<!-- } 배송정보 끝 -->
<?php } ?>
...
<!-- 교환/반품 시작 { -->
<section id="sit_ex">
    <h2><?php echo _t('교환/반품'); ?></h2>
    <?php echo pg_anchor('ex'); ?>

    <?php echo conv_content(_t($default['de_change_content']), 1); ?>
</section>
<!-- } 교환/반품 끝 -->
<?php } ?>
