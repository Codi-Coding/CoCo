...
<?php if ($is_admin) {  //관리자이면 ?>
    <div class="sit_admin"><a href="<?php echo G5_ADMIN_URL; ?>/shop_admin/configform.php#anc_scf_etc" class="btn_admin"><?php echo _t('검색 설정'); ?></a></div>
<?php } ?>
<!-- 검색 시작 { -->
<div id="ssch">
    <!-- 상세검색 항목 시작 { -->
    <div id="ssch_frm">
        <h2><span><strong><?php echo $q; ?></strong> <?php echo _t('검색 결과'); ?></span> (<?php echo _t('총'); ?> <strong><?php echo $total_count; ?></strong> <?php echo _t('건'); ?> )</h2>
        ...
        <div class="ssch_scharea">
            ...
            <div class="ssch_right">
                <strong class="sound_only"><?php echo _t('검색범위'); ?></strong>
                ...
                <input type="checkbox" name="qbasic" id="ssch_qbasic" value="1" <?php echo $qbasic_check?'checked="checked"':'';?>> <label for="ssch_qbasic"><?php echo _t('기본설명'); ?></label>
                ...
                <strong class="sound_only"><?php echo _t('상품가격'); ?> <!--(원)--> (<?php echo $g5['currency_unit_name_list'][$g5['def_currency']]; ?>)</strong>
                <label for="ssch_qfrom" class="sound_only"><?php echo _t('최소 가격'); ?></label>
                <input type="text" name="qfrom" value="<?php echo $qfrom; ?>" id="ssch_qfrom" class="ssch_input" size="10"> <?php echo _t('원'); ?> ~
                <label for="ssch_qto" class="sound_only"><?php echo _t('최대 가격'); ?></label>
                <input type="text" name="qto" value="<?php echo $qto; ?>" id="ssch_qto" class="ssch_input" size="10"> <?php echo _t('원'); ?>
            </div>
            <div class="ssch_left">
                <label for="ssch_q" class="sound_only" ><?php echo _t('검색어'); ?></label>
                <input type="text" name="q" value="<?php echo $q; ?>" id="ssch_q" class="ssch_input" size="40" maxlength="30" placeholder="<?php echo _t('검색어'); ?>">
    ...
    <div id="ssch_cate" class="sct_ct">
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            echo "<li><a href=\"#\" onclick=\"set_ca_id('{$row['ca_id']}'); return false;\">"._t($row['ca_name'])." (".$row['cnt'].")</a></li>\n";
            $total_cnt += $row['cnt'];
        }
    ...
    <div id="sct_sortlst">
        <ul id="ssch_sort">
            <li><a href="#" onclick="set_sort('it_sum_qty', 'desc'); return false;"><?php echo _t('판매많은순'); ?></a></li>
            <li><a href="#" onclick="set_sort('it_price', 'asc'); return false;"><?php echo _t('낮은가격순'); ?></a></li>
            <li><a href="#" onclick="set_sort('it_price', 'desc'); return false;"><?php echo _t('높은가격순'); ?></a></li>
            <li><a href="#" onclick="set_sort('it_use_avg', 'desc'); return false;"><?php echo _t('평점높은순'); ?></a></li>
            <li><a href="#" onclick="set_sort('it_use_cnt', 'desc'); return false;"><?php echo _t('후기많은순'); ?></a></li>
            <li><a href="#" onclick="set_sort('it_update_time', 'desc'); return false;"><?php echo _t('최근등록순'); ?></a></li>
        </ul>
    </div>
    <!-- 검색결과 시작 { -->
    <div>
