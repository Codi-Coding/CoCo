...
<ul id="btn_smt_<?php echo $this->type; ?>" class="sctrl">
    <li><button type="button" class="sctrl_play"><?php echo _t('효과재생'); ?><span></span></button></li>
    <li><button type="button" class="sctrl_stop"><?php echo _t('효과정지'); ?><span></span></button></li>
</ul>
<!-- } 이전 재생 정지 다음 버튼 끝 -->
...
for ($i=1; $row=sql_fetch_array($result); $i++) {
    $row['it_name'] = _t($row['it_name']); /// 2018.03.15
    $row['it_basic'] = _t($row['it_basic']); /// 2018.03.15
    $sct_last = '';
...
if($i == 1) echo "<p class=\"sct_noitem\">i"._t("등록된 상품이 없습니다.")."</p>\n";
?>
        ...
        init: function(option)
        {
            ...
            function set_interval() {
                if(count > 1) {
                    ...
                    $("#btn_"+el_id).find("button span").removeClass("sctrl_on").html("")
                        .end().find("button.sctrl_play span").addClass("sctrl_on").html("<b class=\"sound_only\"><?php echo _t('선택됨'); ?></b>");
                }
            }
            ...
            function clear_interval() {
                if(intervals[el_id]) {
                    ...
                    // control 버튼 class
                    $("#btn_"+el_id).find("button span").removeClass("sctrl_on").html("")
                        .end().find("button.sctrl_stop span").addClass("sctrl_on").html("<b class=\"sound_only\"><?php echo _t('선택됨'); ?></b>");
                }
            }
        },
        stop: function()
...
$(function() {
    ...
    $("#btn_smt_<?php echo $this->type; ?> button.sctrl_stop").on("click", function() {
        if($(this).parent().siblings().find(".sctrl_on").size() > 0) {
            ...
            $(this).children().addClass("sctrl_on").html("<b class=\"sound_only\"><?php echo _t('선택됨'); ?></b>");
            var id = $(this).closest(".sctrl").attr("id").replace("btn_", "");
            $("#"+id).topRolling("stop");
